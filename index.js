...
app.post("/submit-form/locks.php", (req, res) => {
    ...
    new formidable.IncomingForm().parse(req, (err, fields, files) => {
        ...
        if (
            files.openLockImageInput &&
            files.openLockImageInput.originalFilename
        ) {
        ...
        for (let i in rQCards) {
            if (cards[rQCards[i]] && cards[rQCards[i]].name)
                rCards.push(rQCards[i]);
        }
            ...
            for (let i in vCards) {
                if (
                    ...
                    for (let j in users[name].cards) {
                        ...
                        if (users[name].cards[j].name === vCards[i])
                            hasCard = true;
                    }
