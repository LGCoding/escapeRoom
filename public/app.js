...
function navBarClick(that, toPlace) {
    document
        .getElementsByClassName("navActive")[0]
...
function fetchpostLock() {
    ...
    if (keepImageLock) {
        ...
        for (let i in locks) {
            if (
                locks[i].name === document.getElementById("lockNameInput").value
            ) {
                lock = locks[i];
                break;
            }
        ...
        cardBase.addEventListener("click", function () {
            if (isCardEdit) {
                ...
                if (this.classList.contains("bigCard")) {
                    ...
                    this.style.height =
                        parseInt(this.style.height) / 2.5 + "px";
    ...
    if (isCardEdit) {
        setTimeout(function () {
            ...
            for (let i in allCards) {
                ...
                cardBase.addEventListener("click", function () {
                    if (isCardEdit) {
                        document.getElementById("cardMaker").style.display =
                            "block";
                        let card;
                        for (let i in allCards) {
                            if (
                                allCards[i].name === this.getAttribute("namel")
                            ) {
                                card = allCards[i];
                                break;
                            }
    ...
    for (let i in locks) {
        ...
        lockBase.addEventListener("click", function () {
            if (isLockEdit) {
                ...
                document.getElementById("locksResultsRemove").value =
                    lock.toRemove;
                keepImageLock = true;
                isLockEdit = false;
            } else {
